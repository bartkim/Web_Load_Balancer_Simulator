...
public class Simulator extends JFrame implements ActionListener
{
	...
	public Simulator ()
	{
		...
		// graph panel
		throughput = new int[21];
		graph = new Graph(throughput);
	...
	public int start(int numNode, float threshold)
	{
		int i;
		Node temp;
	...
	public void actionPerformed(ActionEvent evt)
	{
		...
		if (source == makefile) {						// makefile Button action
			simulate.setEnabled(false);											// setting simulate button disable
			...
			numLoad = Integer.parseInt( numOfLoad.getText().trim() );				// number of load
			
			makeLoadFile(fileName, numLoad, maxInterval, maxCpu, maxMem, maxBw, maxDuration);		// make load file
						
			simulate.setEnabled(true);											// setting simulate button enable
		}
		else if (source == simulate) {					// simulate Button action
			makefile.setEnabled(false);											// setting makefile button disable

			int numNode = Integer.parseInt( numOfNode.getText().trim() );				// read number of node from text field
			int i;
			
			for ( i = 1 ; i <= 20 ; i++ ) {												// change threshold 0.05 ~ 1
				throughput[i] = start(numNode, (float)(i*0.05));						// call start method..
				//System.out.println (i + "  " + (float)(i*0.05) + "  " + throughput[i]);
			}
			graph.setData(throughput, numLoad);											// send data to graph
			graph.repaint();															// repaint graph
			
			makefile.setEnabled(true);											// setting makefile button enable
		}
	}
	...
	private JButton makefile;		// button
	private JButton simulate;
	
	private Graph graph;			// Panel that draw graph throughput vs threshold
	
	private JTextField interval;	// text field to read interval of load
	private JTextField cpu;			// text field to read interval of load
	private JTextField mem;			// text field to read interval of load
	private JTextField bw;			// text field to read interval of load
	private JTextField dur;			// text field to read interval of load
	private JTextField numOfLoad;	// text field to read interval of load
	private JTextField numOfNode;	// text field to read interval of load
	
	private int numLoad;			// the number of load
	private	int[] throughput;		// to store throughput
